...
# Thresholds
QUICK_MAX_LENGTH = 200       # prompts <= 200 chars -> candidate for quick
GENERAL_MIN_LENGTH = 500     # prompts >= 500 chars -> prefer gemma_full
...
def classify_prompt(prompt):
    ...
    # Priority: code > reasoning > general/quick
    if code_score > reasoning_score and code_score > 0:
        return "code"
    if reasoning_score > 0:
        return "reasoning"

    # Quick vs General: based on prompt length + quick signals
    if prompt_len <= QUICK_MAX_LENGTH or quick_score > 0:
        return "quick"
    return "general"
    ...
    while True:
        ...
        if method == "tools/call":
            ...
            if name == "ask_auto":
                ...
                model_key = TASK_ROUTE.get(task_type, "gemma_full")
                ans, used_model = query_ollama(prompt, model_key)
                header = "[Router: task=%s, model=%s]\n\n" % (task_type, used_model or "N/A")
                ans = header + ans
            else:
